...
export const Login1: React.FC = () => {
  ...
  const onSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    // mock login -> go to profile selection
    navigate('/profiles1')
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-[#FFF8FF] to-[#F4F3F9]">
      <div className="w-full max-w-3xl bg-white rounded-3xl shadow-2xl overflow-hidden grid grid-cols-1 md:grid-cols-2">
        <div className="p-10">
          ...
          <p className="mt-2 text-sm text-[#6B7280]">Sign in to continue to your cinematic story library.</p>
          ...
          <form onSubmit={onSubmit} className="mt-6 space-y-4">
            ...
            <button type="submit" className="w-full py-3 rounded-xl bg-gradient-to-r from-[#7c3aed] to-[#06b6d4] text-white font-semibold">Sign in</button>
          </form>

          <div className="mt-4 text-sm text-[#6B7280]">New? <Link to="/signup1" className="text-[#7c3aed] font-semibold">Create an account</Link></div>
        </div>
        ...
        <div className="hidden md:flex items-center justify-center bg-gradient-to-tr from-[#FDE68A] to-[#FBCFE8] p-6">
          <div className="text-center">
            ...
            <div className="mt-4 text-sm text-[#374151]">Dive into bedtime adventures — safe, curated, and magical.</div>
          </div>
        </div>
      </div>
    </div>
