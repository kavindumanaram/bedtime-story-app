...
import type { DailyStory } from "@bedtime/shared";
import { apiFetch } from "../lib/api";

type StreakSummary = { current_streak: number; longest_streak: number; last_read_date: string | null };
...
export const Dashboard: React.FC = () => {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    if (profile.plan === "basic") {
      try {
        const data = await apiFetch<{ count: number; limit: number }>("/api/quota");
        setQuota(data);
      } catch { /* non-blocking */ }
    }
  }, [activeChild, profile]);
